...
$quer=mysql_query("select * from department where department='$department' and college_faculty='$college_faculty'");
$result=mysql_fetch_array($quer);
$validate=mysql_num_rows($quer);
if ($validate!=0){
	$error1="Department Already EXIST";
}
else{
if ($_POST['submit'] && trim($college_faculty)!="" && trim($department)!=""){
$quer=mysql_query("INSERT into department(college_faculty,department) VALUES('$college_faculty','$department')");
if ($quer==1){
...
$query=mysql_query("SELECT * from college_faculty");
				...
				<div class="mid-grid">
				  ...
				  <p><?php echo $message.$error.$error1;?></p>
					    <form method="post" action="">
					    	<div>
						    	<span><label>Select College/Faculty</span>
						    	<span><select name="college_faculty">
								<?php 
							while($row=mysql_fetch_array($query)){
							?>
								<option><?php echo $row['college_faculty'];?></option>
								<?}?>
								</select></span>
							...
							<div class="clear"></div>
							<div>
						    	<span><label>Enter Department</label></span>
						    	<span><input name="department" type="text" class="textbox"></span>
						    </div>
							<div>
					...
					<li><a href="upload.php">Upload Past Question</a></li>
					</ul>
						</div>
			 	</div>
			 		<div class="mid-grid">
